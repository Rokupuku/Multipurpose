...
import { randomMemberFields } from '../utils/lions';
import { validateLionForm } from '../utils/validation';

function LionForm({ open, onClose, onAdd, submitting }: LionFormProps) {
  const [form, setForm] = useState<LionFormData>(EMPTY_FORM);
  const [errors, setErrors] = useState<LionFormErrors>({});
  ...
  const updateField = (field: keyof LionFormData, value: string) => {
  ...
  const handlePartChange = (e: ChangeEvent<HTMLSelectElement>) => {
    updateField('part', e.target.value as PartValue);
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const nextErrors = validateLionForm(form);
      ...
      <div className="form-modal">
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="form-grid">
            ...
            <label>
              파트 *
              <select value={form.part} onChange={handlePartChange}>
